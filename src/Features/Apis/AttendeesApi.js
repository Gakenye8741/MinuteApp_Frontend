...
export const attendeesApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getAllAttendees: builder.query({
            query: (meetingId) => meetingId ? `Attendees/AllAttendees?meetingId=${meetingId}` : 'Attendees/AllAttendees',
            providesTags: ['Attendees'],
        }),
        // Get single attendee by ID
        getAttendeeById: builder.query({
            query: (id) => `Attendees/AttendeeById/${id}`,
            providesTags: ['Attendees'],
        }),
        // Add a new attendee
...
// ✅ Export hooks
export const { useGetAllAttendeesQuery, useGetAttendeeByIdQuery, useAddAttendeeMutation, useUpdateAttendeeMutation, useUpdateAttendeeStatusMutation, useDeleteAttendeeMutation, } = attendeesApi;
